...
private:
    ...
    double a(double t)
    {
        ...
        for(int i=0; i<f_c.size(); i++){
            ...
            A_i = (a < A_c.size()) ? A_c[i].toDouble() : 0.9 / f_c.size(); //rovnomerne 90% plneni pokud neni zadano jinak
            A += A_i * cos(2*M_PI*f_c[i]*t);
        }
...
public:
    t_rt_multisin_generator(const QString &js_config, QObject *parent = NULL):
      t_rt_generator(js_config, std::bind(&t_rt_sweep_generator::a, this, std::placeholders::_1), parent)
    {
        A_c = par["A_c"].get().toArray();
        f_c = par["f_c"].get().toArray();
        fs = par["fs"].get().toInt();
    }
